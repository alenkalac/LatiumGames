...
if ( $PaymentOption == "PayPal" )
{
	...
	if( $ack == "SUCCESS" || $ack == "SUCCESSWITHWARNING" )
	{
		...
		$email				= $resArray["EMAIL"]; // Unique transaction ID of the payment.
	...
	//	$exchangeRate		= $resArray["PAYMENTINFO_0_EXCHANGERATE"];  // Exchange rate if a currency conversion occurred. Relevant only if your are billing in their non-primary currency. If the customer chooses to pay with a currency other than the non-primary currency, the conversion occurs in the customer's account.
		...
		$stid = $_GET['tid'];
		echo "stid= " . $stid;
		$query = "UPDATE transactions_new SET paypal_token = '$token', paypal_payerID = '$payerID', email = '$email' WHERE transactionid='$stid'";
		echo $query;
		$result = $link->query($query) or die("Something wrong with database");
		
		//redirect the user to a new page where the IPN will be checked against this payment, if they match up. payment has been successfully completed.
		header(location: "../confirm.php?tid=$stid");
